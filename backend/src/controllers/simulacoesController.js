import { buscarSimulacoesExternas } from "../services/apiService.js";
...
export async function obterSimulacoes(req, res) {
    try {
        const { base, usuario, senha, data } = req.body;
        ...
        const simulacoes = await buscarSimulacoesExternas({
            base,
            usuario,
            senha,
            data,
        ...
        console.log("Qtd registros retornados:", Array.isArray(simulacoes) ? simulacoes.length : "não é array");

        const resumo = calcularResumo(simulacoes);

        // devolve resumo + alguns dados brutos para debug
        return res.json({
            ...resumo,
            qtdRegistros: Array.isArray(simulacoes) ? simulacoes.length : 0,
            exemploPrimeiraRota: Array.isArray(simulacoes) && simulacoes.length > 0 ? simulacoes[0] : null,
        });
    } catch (error) {
        console.error("Erro ao obter simulações:", error.message);
